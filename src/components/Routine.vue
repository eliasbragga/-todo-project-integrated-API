<template>
  <div>
    <v-btn color="#1976D2" class="mt-4" @click="showTimeline()">Gerar rotina</v-btn>
    <v-dialog persistent width="900" v-model="dialog">
      <v-card >
        <v-container grid-list-md class="container-timeline" pa-0>
          ...
          <v-layout row wrap v-if="!requestEnd" >
          ...
          >
            ...
            <Timeline :todoList="todoList"/>
            <v-flex xs12 class="text-center mt-5">
              <v-btn color="#E1B17A" @click="hideTimeline()"> fechar </v-btn>            
            </v-flex>
          </v-layout>
        </div>
        </v-container>
...
export default Vue.extend({
  ...
  methods: {
    ...
    apiChatGPT() {
      let prefix = ` lista de tarefas do dia ${this.itensTodo}. Faça um cronograma para minhas tarefas para que eu realize da melhor forma possível em um dia. Faça uma descrição curta para cada item fazendo uma breve descrição e se o tema for sobre estudo, me diga o que devo estudar. mande o texto formatado com linhas quebradas com emoticos legais que tenham haver com as atividades e me motive a realiza-las. Após o cronograma faça um breve muito curto texto sobre a importancia das atividades que lhe listei e me motive a realiza-las. "`;
